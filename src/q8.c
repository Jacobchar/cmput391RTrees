...
void insertIntoNearest(double, int);
void sortBranchList(NodeStuff *, int);
void sortNearestList();
...
int main(int argc, char **argv) {
	...
	if (argc != 4) {
	  ...
	  return FAILURE;
		...
		y = atof(argv[2]);
		k = atoi(argv[3]);
		nearest = (Nearest*)malloc(sizeof(Nearest) * k);

		int i;
		for(i = 0; i < k; i++) {
			nearest[i].dist = INFINITY;
		}
...
void nearestNeighbourSearch(NodeStuff *Node, int depth){
	...
	if(depth == rtreeDepth) {	
		...
		int *leaves = (int*)malloc(sizeof(int) * leafCount);
		getLeaves(db, stmt, leafCount, leaves, Node);
		sortNearestList();
		for(i = 0; i < leafCount; i++) {
			dist = leafDist(db, stmt, leaves[i]);
			insertIntoNearest(dist, leaves[i]);
		}
		free(leaves);

	} else {

		NodeStuff *ActiveBranchList = (NodeStuff*)malloc(sizeof(NodeStuff) * 1000);
...
void insertIntoNearest(double dist, int leafID) {
	
	int i;

	if(dist > nearest[k-1].dist) {
		return;
	} else if(dist <= nearest[k-1].dist) {
		for(i = 0; i < k; i++) {
			
			if(dist < nearest[i].dist) { 			
				int size = k - 1;
				while(size > i) {
					nearest[size] = nearest[size - 1];
					size--;
				}
				nearest[i].dist = dist;
				nearest[i].id = leafID;
		...
		nearest[k-1].dist = dist;
		nearest[k-1].id = leafID;
		return;
	}
}
...
void sortNearestList() {
	...
	for(i = 0; i < k-1; i++) {
		...
		for(j = 0; j < k-1-i; j++) {
			if(nearest[j].dist > nearest[j+1].dist){
				temp = nearest[j];
...
int uppruneBranchList(NodeStuff *ABL, int last_index) {
	...
	int i;
	for(i = 0; i < last_index; i++) {
		if(ABL[i].minDist > nearest[k-1].dist) {
			return i;
		}
	}
	return 0;
...
void printNearestList() {
	int i = 0;
	for(i = 0; i < k; i++) {
		printf("%d - %f\n", nearest[i].id, nearest[i].dist);
	}
	printf("-----------------------\n");
}